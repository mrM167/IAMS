...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        ...
        if ($emailChk->fetch()) {
            $errors[] = 'This email address is already registered. Please use a different email or <a href="/login.php">log in</a>.';
        }
        ...
        if ($studentChk->fetch()) {
            $errors[] = 'Student number <strong>' . htmlspecialchars($data['student_number']) . '</strong> is already registered. Each student can only have one account.';
        }
    }
...
<div class="card">
  ...
  <div class="card-body">
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <input type="text" name="programme" required 
                 value="<?php echo htmlspecialchars($data['programme'] ?? ''); ?>" 
        ...
        <div class="form-group">
          ...
          <select name="year_of_study" required>
            ...
            <?php for ($y = 1; $y <= 6; $y++): ?>
            <option value="<?php echo $y; ?>" <?php echo ($data['year_of_study'] ?? 0) == $y ? 'selected' : ''; ?>>
              Year <?php echo $y; ?>
            </option>
            <?php endfor; ?>
          </select>
    ...
    <div class="footer-links">
      ...
      &nbsp;|&nbsp; 
      <a href="/register_org.php">Register as Organisation</a>
    </div>
  </div>
</div>
