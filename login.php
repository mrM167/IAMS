...
if (isLoggedIn()) {
    $role = $_SESSION['role'];
    header('Location: ' . ($role === 'student' ? '/dashboard.php' : ($role === 'organisation' ? '/org/dashboard.php' : '/admin/index.php')));
    exit();
}
    ...
    if (!$email || !$password) {
        ...
        if ($result['ok']) {
            ...
            $dest = match($role) {
                ...
                'organisation' => '/org/dashboard.php',
...
.login-footer{text-align:center;margin-top:1.5rem;font-size:.85rem;color:#6b7280}
...
.toggle-pw{position:absolute;right:.75rem;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:1rem;color:#9ca3af}
</style>
</head>
<body>
<div class="login-card">
  <div class="login-top">
    <svg width="50" height="50" viewBox="0 0 50 50" style="margin-bottom:.75rem"><circle cx="25" cy="25" r="23" fill="none" stroke="#c9a84c" stroke-width="2"/><text x="25" y="31" text-anchor="middle" font-family="Georgia,serif" font-size="14" font-weight="bold" fill="white">UB</text></svg>
    <h1>IAMS</h1>
    <p>"We give the pathway to future leaders"</p>
  </div>
  <div class="login-body">
    <?php if ($timeout): ?>
    <div class="alert-warning">⏱ Your session expired due to inactivity. Please log in again.</div>
    <?php endif; ?>
    <?php if ($error): ?>
    ...
    <form method="POST">
      ...
      <div class="form-group">
        ...
        <div class="password-wrapper">
          <input type="password" name="password" id="pw" required autocomplete="current-password" placeholder="••••••••">
    ...
    <div style="text-align:center;margin-top:.75rem"><a href="/index.php" style="font-size:.8rem;color:#9ca3af;text-decoration:none">← Back to home</a></div>
  </div>
</div>
<script>
function togglePw(){
