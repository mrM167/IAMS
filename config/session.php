<?php
// config/session.php — Secure session management
...
// ── Regenerate session ID periodically (session fixation prevention) ──
...
function renderFlash(): string {
    ...
    foreach (['success','error','info','warning'] as $t) {
        $msg = getFlash($t);
        if ($msg) {
            $colors = ['success'=>'#d4edda;color:#155724','error'=>'#f8d7da;color:#721c24',
                       'info'=>'#d1ecf1;color:#0c5460','warning'=>'#fff3cd;color:#856404'];
            $out .= '<div style="background:' . $colors[$t] . ';padding:.75rem 1rem;border-radius:8px;margin-bottom:1rem;">'
